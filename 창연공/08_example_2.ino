...
void loop() {
  ...
  if (distance > _DIST_MAX || distance < _DIST_MIN) using_distance = before_distance;
  else using_distance = distance;

  if (using_distance < _DIST_MIN) {
    distance = _DIST_MIN - 10.0;    // Set Lower Value
    analogWrite(PIN_LED, 255);
  } else if (using_distance > _DIST_MAX) {
    distance = _DIST_MAX + 10.0;    // Set Higher Value
    analogWrite(PIN_LED, 255);       // LED OFF
  } else {    // In desired Range
    if(using_distance > 200) flag = 300;
    led_bright = 255 - (51.0 / 2000.0 * (using_distance-flag) * (using_distance-flag));
  ...
  Serial.print(",led_bright:");       Serial.print(led_bright);
  Serial.println("");
